...
class NatsStreamingFlowSpec extends AnyWordSpec
  ...
  override def subjectName: String = "NatsStreamingFlowSpec"
  ...
  "Nats Streaming Flow Stage" should {
    ...
    "publish message with ack" in{
      val result: Future[Message] = NatsStreamingSource.subscribe(natsSubscriberSettings,streamingConnection).runWith(Sink.head)

      val publishResult: Future[model.NatsPublishResult[String]] = Source.single(NatsPublish(subjectName,message.getBytes(StandardCharsets.UTF_8)))
        .via(NatsStreamingFlow.publishWithAck(1,streamingConnection)).runWith(Sink.head)

      result.futureValue(timeout)

      whenReady(publishResult.map(_.result.get)){guid =>
        guid.length > 0 shouldBe true
      }
    }
